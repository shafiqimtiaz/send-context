...
> **In one terminal, send:** `CTX_HANDOFF_WORKER=your-project.deno.net ctx-handoff export` — share the link out of band, share the password separately.
>
...
- **Password.** Never part of the link, never sent over the wire, never stored. Minimum 4 characters (enforced by the export prompt). Sender and receiver share it out of band.
...
Without `GEMINI_API_KEY`, `export` walks you through these prompts:
...
| `Link expired or invalid.` | The 24-hour TTL elapsed, the id is wrong, or the worker host is unreachable. | Re-export from the sender. |
...
| `Gemini request failed (NNN): …` | Gemini call errored (rate limit, bad key, network). | Distillation auto-falls-back to manual — re-run `export`; the manual flow is unaffected. |
...
`pi` and Claude Code are detected by checking whether the corresponding project folder exists; if both are present, you'll be asked which one to export from. OpenCode is detected by the `opencode` binary being on `PATH`.
...
src/
  ...
  commands/               export (CLI shell), receive
worker/
